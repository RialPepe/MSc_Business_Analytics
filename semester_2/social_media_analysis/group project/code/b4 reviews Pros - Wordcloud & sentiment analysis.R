...
pros <- b4_data$pros
pros_source <- VectorSource(b4_data$pros)
pros_corpus <- VCorpus(pros_source)
...
clean_corp <- clean_corpus(pros_corpus)

pros_tdm <- TermDocumentMatrix(clean_corp)

pros_m <- as.matrix(pros_tdm)

term_frequency <- rowSums(pros_m)
...
#Word Association 
associations <- findAssocs(pros_tdm, terms = c("people", "opportunities", "culture", "learning", "environment",
                                               "career", "experience", "benefits","place", "learn"), corlimit = 0.2)
associations

# Get sentiment scores using afinn package
afinn_vector <- get_sentiment(pros, method="afinn")
