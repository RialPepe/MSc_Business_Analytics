...
library(corrplot)
library(ggplot2)
Attrition <-read.csv("C:/Users/pepec/Desktop/estudios/master/semester_1/business_forecasting/group assignment/attrition.csv")
library(ggplot2)
library(car)
attrn<-(Attrition)
```
...
```{r, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
data<- subset(attrn, select=-c(EmployeeCount,EmployeeNumber,Over18,StandardHours))
dim(data)
names(data)
```
...
```{r, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
ggplot(data=data, aes(Attrition,fill = Attrition))+geom_histogram(stat="count")+labs(title="Employee Attrition (Count)", x="Employee Attrition",y="count")
```
...
num_cols
sub<-subset(data, select=c(Attrition, Age,DailyRate,
...
```{r, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
# Convert 'Attrition' factor to 0 and 1
...
```{r, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
g1<-glm (Attrition ~ ., data=train, family = 'binomial')
summary(g1)
```
...
```{r, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
g1.step.inter<- glm(Attrition~(Age + BusinessTravel + DailyRate + Department + EducationField + 
...
```{r, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
g2<-glm(Attrition ~ (Age + BusinessTravel + DailyRate + Department + EducationField + 
    EnvironmentSatisfaction + Gender + JobInvolvement + JobLevel + 
    JobRole + JobSatisfaction + MonthlyIncome + NumCompaniesWorked + 
    OverTime + RelationshipSatisfaction + StockOptionLevel + 
...
```{r, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
g3<- glm(Attrition ~ Age+ BusinessTravel+ EducationField+ EnvironmentSatisfaction+ JobInvolvement+ JobSatisfaction+ OverTime+ StockOptionLevel+ YearsSinceLastPromotion, data= train)
g3.step<- step(g3)
#summary(g3.step)
```
